...
function rewriteTarget(target, externalHost) {
  ...
  wsFields.forEach((field) => {
    if (rewritten[field] && typeof rewritten[field] === "string") {
      ...
      if (originalUrl !== rewritten[field]) {
        console.log(
          `🔄 Rewrote WebSocket ${field}: ${originalUrl} → ${rewritten[field]}`
        );
      }
    }
  });
  ...
  httpFields.forEach((field) => {
    if (rewritten[field] && typeof rewritten[field] === "string") {
      ...
      if (
        rewritten[field].includes("devtools") ||
        ...
        if (originalUrl !== rewritten[field]) {
          console.log(
            `🔄 Rewrote HTTP ${field}: ${originalUrl} → ${rewritten[field]}`
          );
        }
      }
    }
...
async function waitForChrome() {
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      console.log(
        `⏳ Waiting for Chrome CDP... (attempt ${i + 1}/${maxRetries})`
      );
  ...
  try {
    ...
    return new Promise((resolve) => {
      const socket = net.createConnection(
        { port: CHROME_PORT, host: "127.0.0.1" },
        () => {
          socket.end();
          resolve(true);
        }
...
function startChrome() {
  ...
  try {
    ...
    execSync(
      'pgrep Xvfb || (echo "Starting Xvfb" && Xvfb :99 -screen 0 1920x1080x24 -ac &)'
    );
    execSync(
      'for i in {1..5}; do if [ -e /tmp/.X11-unix/X99 ]; then echo "X server ready"; break; fi; sleep 1; done'
    );
  } catch (e) {
    console.warn(
      "Warning: X server check failed, Chrome might not start properly"
    );
  }
  const chromeCmd =
  ...
  const chromeArgs = [
    // Core container/security flags
    "--no-sandbox",
    "--test-type", // Required for CDP functionality
    ...
    // Anti-detection - data collection
    "--disable-sync",
    ...
    "--enable-automation", // Keep this for CDP functionality
...
server.on("upgrade", (req, socket, head) => {
  ...
  chromeReq.on("upgrade", (chromeRes, chromeSocket, chromeHead) => {
    ...
    socket.write(
      `HTTP/1.1 ${chromeRes.statusCode} ${chromeRes.statusMessage}\r\n`
    );
